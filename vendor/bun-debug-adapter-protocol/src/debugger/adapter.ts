...
export class WebSocketDebugAdapter extends BaseDebugAdapter<WebSocketInspector> {
  ...
  async #launch(request: LaunchRequest): Promise<void> {
    ...
    // Bun re-creates its inspector server on every `--watch`/`--hot` reload. A unix-domain
    // socket cannot be re-bound while the previous listener is still alive in the same
    // process, so a watching debuggee dies with EADDRINUSE from `internal:debugger`.
    // TCP rebinds cleanly, so watch sessions always use it. (Local patch, see VENDOR.md.)
    const useUnixSocket = process.platform !== "win32" && !watchMode;

    // 127.0.0.1 rather than localhost so it resolves correctly on windows.
    const url = useUnixSocket
      ? `ws+unix://${randomUnixPath()}`
      : `ws://127.0.0.1:${await getAvailablePort()}/${getRandomId()}`;
    const signal = useUnixSocket ? new UnixSignal() : new TCPSocketSignal(await getAvailablePort());
